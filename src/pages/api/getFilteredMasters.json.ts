...
export const GET: APIRoute = async ({ request }) => {
	...
	if (school && school !== "all") {
		filters.push((data: any) => data.general.school === school);
	}

	const afterSchool = performance.now();

	// SubSchool
	if (subSchool && subSchool !== "all") {
		if (school !== "all") {
			filters.push((data: any) => data.general.subSchool === subSchool);
		} else {
			return new Response(JSON.stringify({ error: "Please provide a school" }), {
				status: 400,
				headers: { "content-type": "application/json" },
			});
		}
	...
	const afterSubSchool = performance.now();
	...
	const allMasters = await getCollection("masters", ({ data }: { data: any }) => {
	...
	if (!allMasters || allMasters.length === 0) {
		return new Response(JSON.stringify({ error: "No masters found" }), {
			status: 404,
			headers: { "content-type": "application/json" },
		});
	}
	...
	const performanceResults = {
		...
		School: afterSchool - afterParsing,
		SubSchool: afterSubSchool - afterSchool,
		Price: afterPrice - afterSubSchool,
